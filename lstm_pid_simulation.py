...
class LSTMPIDTuner(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, output_size, window_size=10):
        ...
        # 3. Actor Head (Outputs PID gains)
    ...
    def forward(self, x):
        """Actor forward pass: Returns only the PID gains for GUI backwards compatibility"""
        last_out = self.extract_features(x)
        
        actor_out = F.relu(self.actor_fc1(last_out))
        actor_out = self.actor_fc2(actor_out)
        return self.softplus(actor_out)
    ...
    def get_action_and_value(self, x):
        ...
        actor_out = F.relu(self.actor_fc1(last_out))
        actor_out = self.actor_fc2(actor_out)
        action = self.softplus(actor_out)
...
if __name__ == "__main__":
    ...
    print("Predicted PID Gains shape:", predicted_gains.shape)
